...
<ul class="toc-level-<?= $tree[0]['level'] ?>">
    <?php foreach ($tree as $item): $active_level = $destination == $item['destination'];?>
        <li class="<?= $active_level ? "active" : "inactive" ?>">
            ...
            <?php if(!empty($item['children']->u())) {
                print $this->partial('table_of_contents_tag', 
                    ['tree' => $item['children'], 'min_level' => $min_level, 'max_level' => $active_level ? 10 : $max_level, 'destination' => $destination]
                ); 
            }?>
        </li>
    <?php endforeach; ?>
